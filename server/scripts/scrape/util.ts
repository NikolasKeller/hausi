...
// run clubs, coffee meetups, dating events, raves, rooftop parties, matcha
// pop-ups, pilates/yoga, social sports … Each group maps to an app category.
const KEYWORD_CATEGORIES: { category: string; theme: string; words: RegExp }[] = [
  { category: 'sports', theme: 'forest', words: /\b(run(ning)? ?club|run club|social run|city runners?|morning run|sunset run|5k|10k|parkrun|trail run)\b/i },
  { category: 'sports', theme: 'matcha', words: /\b(yoga|pilates|breathwork|meditation|wellness|sound ?bath|cold ?plunge|ice ?bath|sauna|hyrox|bootcamp|workout|fitness|calisthenics)\b/i },
  { category: 'sports', theme: 'ocean', words: /\b(spikeball|roundnet|padel|volleyball|beach ?volley|basketball|pickleball|bouldern?|bouldering|climbing|skate|surfskate|hike|hiking|bike ride|cycling|swim)\b/i },
  { category: 'food', theme: 'matcha', words: /\b(matcha|coffee ?(meetup|rave|club|social|morning)|café hopping|cafe crawl|brunch(?! business)|supper ?club|tasting|brew|barista|picnic|pop[- ]?up)\b/i },
  { category: 'community', theme: 'candy', words: /\b(dating|singles?|speed ?dating|blind date|mixer|match ?making|matchmaking|crush|first dates?|hinge|bumble|tinder)\b/i },
  { category: 'community', theme: 'peach', words: /\b(meetup|social club|networking|new in town|make friends|community|language exchange|expat|erasmus|international people|board ?games?|book ?club|walking club|creative club)\b/i },
  { category: 'music', theme: 'midnight', words: /\b(techno|rave|club ?night|house music|dj set|open air|day ?party|rooftop|boat party|afterparty|warehouse|disco|hard ?groove|melodic|trance|drum ?& ?bass|dnb|hip ?hop night|r&b night)\b/i },
  { category: 'arts', theme: 'lavender', words: /\b(life drawing|paint (&|and) sip|sip (&|and) paint|pottery|ceramics|art class|craft|figure drawing|gallery|vernissage|open mic|poetry|journaling)\b/i },
];
...
export function classify(title: string, description: string): Classified | null {
  ...
  for (const k of KEYWORD_CATEGORIES) {
    if (k.words.test(text)) {
      ...
      return { category: k.category, coverTheme: k.theme };
    }
  }
  return null;
}
