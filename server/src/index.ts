...
// Ticket PDFs generated by the purchase agent. Same traversal-safe filename
// rule as images; served inline so the app can open them in a viewer tab.
app.get('/uploads/tickets/:name', async (c) => {
  const name = c.req.param('name');
  if (!/^[a-zA-Z0-9_-]+\.pdf$/.test(name)) return c.notFound();
  try {
    const buffer = await readFile(join(TICKET_DIR, name));
